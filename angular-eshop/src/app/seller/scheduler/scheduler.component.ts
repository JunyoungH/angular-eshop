...
export class SchedulerComponent implements OnInit, AfterViewInit  {
  ...
  @ViewChildren(SchedulerItemComponent) schedulerItem:QueryList<SchedulerItemComponent>;
  ...
  updateFlightDate(){
    this.setflightDate();
    this.schedulerItem.forEach((val,key)=> val.updateFlightDate(this.flightDate[key], this.scheduler.flightInterval));
    this.updateStoredDate();
  }
